            ...
            ExternalLogins = (await _signInManager.GetExternalAuthenticationSchemesAsync()).ToList();
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        if (!await _roleManager.RoleExistsAsync(SD.TeamMember))
                        {
                            await _roleManager.CreateAsync(new IdentityRole(SD.TeamMember));
                        }
                        if (!await _roleManager.RoleExistsAsync(SD.HOD))
                        {
                            await _roleManager.CreateAsync(new IdentityRole(SD.HOD));
                        }

                        if (role == SD.HOD)
                        {
                            await _userManager.AddToRoleAsync(user, SD.HOD);
                        }
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    await _userManager.AddToRoleAsync(user, SD.TeamMember);
